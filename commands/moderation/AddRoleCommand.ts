...
export default class AddRole extends Command {
    ...
    async Execute(interaction: ChatInputCommandInteraction) {
        ...
        if (!interactionMember?.permissions.has(new AddRole(this.client).default_member_permissions)) return await interaction.reply({ embeds: [new ErrorEmbed("You do not have permissions to execute this command.")], ephemeral: true });
        ...
        try {
            ...
            await interaction.reply({ embeds: [new SuccessEmbed(`The role ${role} has been correctly granted to ${target}.`)], ephemeral: true });
        } catch (error) {
            console.error(error);
            await interaction.reply({ embeds: [new ErrorEmbed(`An error occurred adding the role ${role} to the user ${target}.`)], ephemeral: true });
        }
    }
}